...
from rosdata.msg import Heading
import utm
import math
...
TriggerCustomer = pd.DataFrame(columns=['triggerId', 'timestamp'])
...
def callback_trigger(data):
    ...
    TriggerCustomer.loc[iT] = [int(data.seq), data.stamp.to_sec()]
    iT += 1


# inertial gps fix part

def callback_inertial_gps_fix(data):
    ...
    # print y
    # print x
    GPSInfoShop.loc[iG] = [data.header.stamp.to_sec(), data.latitude, data.longitude, y, x]
    iG += 1
...
def callback_heading_imu_data(msg):
    """
    / *
    ...
    * /
    ...
    z = msg.orientation.z
    w = msg.orientation.w
    yaw, pitch, roll = calcRollPitchYaw(w, x, y, z)
    ...
    OrientationShop.loc[iImu] = [msg.header.stamp.to_sec(), yaw, pitch, roll]
    iImu += 1
